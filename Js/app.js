  ...
  DibujarTallo();
    ...
    function dibujarSiguientePetalo() {
        if (contadorPetalos <= NumeroPetalos) {
          ...
          DibujarPetalo(x, y, RadioXPetalo, 2, Angulo, 'rgb(255, 21, 216)', 100);
          contadorPetalos++;
  ...
  DibujarTallo();
    ...
    function dibujarSiguientePetalo() {
      if (contadorPetalos < NumeroPetalos) {
        ...
        ctx.shadowOffsetX = 0; // Distancia horizontal de la sombra
        ctx.shadowOffsetY = 0; // Distancia vertical de la sombra
        ctx.fill();
        
        // Dibuja un círculo sobre el centro para aplicar la sombra
        ctx.beginPath();
        ctx.arc(x, y, 10, 0, Math.PI * 2);
        ctx.fill();
      }
...
function CrearVarias() {
    ...
    const espacioY = canvas.height / 6;
    const TamañoFlor = 0;
